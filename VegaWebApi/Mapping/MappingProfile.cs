    ...
    public class MappingProfile : Profile
    {
        public MappingProfile()
        {
            ...
            CreateMap<SaveVehicleResource, Vehicle>()
                ...
                .AfterMap((vr, v) => {
                    //Eliminar 'features' que no están en el modelo
                    /*Forma LINQ */
                    var removedFeatures = v.Features.Where(f => !vr.Features.Contains(f.FeatureId));
                    foreach (var f in removedFeatures) 
                        v.Features.Remove(f);
                    
                    //Agrega nueva 'feature' a la lista
                    /*Forma LINQ */
                    var addedFeatures = vr.Features.Where(x => !v.Features.Any(f => f.FeatureId == x)).Select(id => new VehicleFeature { FeatureId = id});
